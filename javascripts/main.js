function gainStone(p1) {
    ...
    if (player.particles >= player.stoneCost * p1) {
        ...
        player.particles = 0;
        if (player.serenityMilestones[3].bought === true) {
            resetSerenity();
...
function buyStoneGenerator(i) {
    ...
    if (player.stone >= sg.cost) {
        ...
        sg.bought += 1;
        sg.amount += 1;
        sg.mult = Math.max(1,(player.stoneGeneratorExp ** (sg.bought - 1)) * player.totalSerenitiesMult);
        sg0.mult = Math.max(1,(player.stoneGeneratorExp ** (sg0.bought - 1)) * player.stoneUpgrades[1].mult * player.totalSerenitiesMult);
...
function buyMaxAllStoneGenerators() {
    for(let i = 0; i < 10; i++) {
        if ((player.stoneGenerators[i].bought === 0) && (player.stone >= player.stoneGenerators[i].cost)) {
            buyMaxStoneGenerator(i);
        }
        //buyMaxStoneGenerator(i);
    }
    for(let u = 0; u < 10; u++) {
        if (player.stone >= player.stoneGenerators[u].cost) {
            buyMaxStoneGenerator(u);
        }
    }
    /*
    for(let u = 0; u < 10; u++) {
...
function toggleDetailedStoneGenerators() {
    if (player.stoneGeneratorsDetailed === true) {
        ...
        player.stoneGeneratorsDetailed = true;
...
var stoneUpgrade1 = {
    ...
    bought: 0,
    mult: 1,
    costIncrease: '(1e6 ** su.bought) * 1e6',
    costIncreaseExp: 1e6,
    displayed: false,
}
...
function buyMaxAllStoneUpgrades() {
    if (player.serenityMilestones[1].bought === true) {
        buyMaxStoneUpgrade(1);
    }
    buyMaxStoneUpgrade(0);
}

function toggleStoneGeneratorAutobuyer() {
...
function toggleStoneUpgradeAutobuyer() {
    if (player.stoneUpgradesAutobuyer === true) {
        ...
        player.stoneUpgradesAutobuyer = true;
